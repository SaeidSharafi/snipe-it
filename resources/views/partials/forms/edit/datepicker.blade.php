<!-- Datepicker -->
<div class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}" id="datepicker-app">
    {{ Form::label($fieldname, $translated_name, array('class' => 'col-md-3 control-label')) }}
    <div class="input-group col-md-3">
        <x-date-picker name="{{ $fieldname }}"
                       id="{{ $fieldname }}"
                       initial-value="{{ old($fieldname, ($item->{$fieldname}) ? $item->{$fieldname}->format('Y-m-d') : '') }}"
